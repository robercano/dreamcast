...
int GenerateOpcodeLUT(const char *basename)
{
	...
	for (i=0; i<sizeof opcodeDefs/sizeof *opcodeDefs; ++i) {
		...
		fprintf(cDisFile, "{\n");
		fprintf(cDisFile, "    /* ");
				fprintf(cDisFile, opcodeDefs[i].mnemonic, opcodeDefs[i].seenVars[opcodeDefs[i].seenVarsOrder[0]],
		...
		for (j=0; j<opcodeDefs[i].numSeenVars; ++j) {
			fprintf(cDisFile, ", %c", opcodeDefs[i].seenVars[opcodeDefs[i].seenVarsOrder[j]].symbol);
		}
		fprintf(cDisFile, ");\n");
		fprintf(cDisFile, "}\n");
		fprintf(cDisFile, "\n");
	...
	for (i=0; i<sizeof opcodeDefs/sizeof *opcodeDefs; ++i) {
		...
		fprintf(cEmuFile, "{\n");
		fprintf(cEmuFile, "    /* ");
				fprintf(cEmuFile, opcodeDefs[i].mnemonic, opcodeDefs[i].seenVars[opcodeDefs[i].seenVarsOrder[0]],
						                                opcodeDefs[i].seenVars[opcodeDefs[i].seenVarsOrder[1]],
						                                opcodeDefs[i].seenVars[opcodeDefs[i].seenVarsOrder[2]]);
		fprintf(cEmuFile, " */\n");
...
int main(int argc, char **argv)
{
	...
	if (GenerateOpcodeLUT(argv[1]) != 0) {
		fprintf(stderr, "ERROR generating the files\n");
		exit(1);
	}
	fprintf(stdout, "Done!\n");
	return 0;
